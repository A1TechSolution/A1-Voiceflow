...
The generated files live under `a1_voiceflow_sdk.typed` and are committed to
the repository so `pip install a1-voiceflow-sdk` ships typed classes without
requiring a regen step.

Run manually:

    python -m a1_voiceflow_sdk.codegen --api http://localhost:8000 \\
        --out sdk/python/src/a1_voiceflow_sdk/typed

    python -m a1_voiceflow_sdk.codegen --input specs.json \\
        --out ./my_typed
"""
...
def _render_init_module(spec_names: list[str]) -> str:
    lines = [
        ...
        "`from a1_voiceflow_sdk.typed import StartCall, AgentNode`.",
        '"""',
        "",
    ]
    exports: list[str] = []
    for spec_name in sorted(spec_names):
        ...
        lines.append(f"from a1_voiceflow_sdk.typed.{module_name} import {class_name}")
        exports.append(class_name)

    lines.append("from a1_voiceflow_sdk.typed._base import TypedNode")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m a1_voiceflow_sdk.codegen",
        description="Generate typed SDK dataclasses from the A1 Voiceflow node-spec catalog.",
    )
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--api", help="A1 Voiceflow backend base URL")
