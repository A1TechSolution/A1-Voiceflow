                ...
                const response = await createOrUpdateEmbedTokenApiV1WorkflowWorkflowIdEmbedTokenPost({
                    ...
                    body: {
                        ...
                        settings: {
                            ...
                            size: "medium",
                            autoStart: false,
                            containerId: embedMode === "inline" ? "a1-voiceflow-inline-container" : undefined,
                        },
                        usage_limit: null,
                                        ...
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                ...
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    ...
                                                    <li>• Call <code className="text-xs">window.A1 VoiceflowWidget.start()</code> to begin a call.</li>
                                                    <li>• Call <code className="text-xs">window.A1 VoiceflowWidget.end()</code> to end it.</li>
...
window.A1 VoiceflowWidget?.onStatusChange((status) => {
  callStatus = status;
...
document.getElementById('talk-btn').addEventListener('click', () => {
  if (callStatus === 'connected' || callStatus === 'connecting') {
    window.A1 VoiceflowWidget.end();
  } else {
    window.A1 VoiceflowWidget.start();
  }
});`}</code>
                                                </pre>
  ...
  useEffect(() => {
    window.A1 VoiceflowWidget?.onStatusChange(setStatus);
  }, []);

  const isLive = status === 'connected' || status === 'connecting';
  return (
    <button onClick={() => isLive ? window.A1 VoiceflowWidget.end() : window.A1 VoiceflowWidget.start()}>
                                    ...
                                    {embedMode === "inline" && (
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                <h4 className="font-medium mb-2">Integration Instructions</h4>
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    <li>• Add a div with id=&quot;a1-voiceflow-inline-container&quot; where you want the widget</li>
                                                    <li>• The widget will render inside this container</li>
                                                    <li>• You have full control over the container&apos;s styling</li>
                                                    <li>• Call window.A1 VoiceflowWidget.start() to begin the call</li>
                                                    <li>• Call window.A1 VoiceflowWidget.end() to end the call</li>
                                                </ul>
                                            </div>
                                            ...
                                            <div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 p-4 border border-blue-200 dark:border-blue-800">
                                                ...
                                                <pre className="text-xs overflow-x-auto">
                                                    <code className="text-blue-800 dark:text-blue-200">{`export function A1 VoiceflowAgent() {
  const [isCallActive, setIsCallActive] = useState(false);

  useEffect(() => {
    // Widget will auto-initialize when script loads
    window.A1 VoiceflowWidget?.onCallStart(() => {
      setIsCallActive(true);
    });
    window.A1 VoiceflowWidget?.onCallEnd(() => {
  ...
  return (
    <div className="my-8">
      ...
      <div id="a1-voiceflow-inline-container" className="min-h-[400px]">
        {/* Widget renders here */}
      </div>
      <button
        onClick={() => window.A1 VoiceflowWidget?.start()}
