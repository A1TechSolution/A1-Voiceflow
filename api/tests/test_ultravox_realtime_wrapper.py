...
def _make_service() -> A1 VoiceflowUltravoxRealtimeLLMService:
    service = A1 VoiceflowUltravoxRealtimeLLMService(
...
async def test_system_instruction_update_marks_reconnect_required():
    ...
    changed = await service._update_settings(
        A1 VoiceflowUltravoxRealtimeLLMService.Settings(system_instruction="new instruction")
    )
...
def test_build_one_shot_params_uses_explicit_greeting_text():
    ...
    params = service._build_one_shot_params(
        greeting_text="Welcome to A1 Voiceflow",
        initial_messages=None,
        agent_speaks_first=True,
    )

    assert params.extra["firstSpeakerSettings"] == {
        "agent": {"text": "Welcome to A1 Voiceflow"}
    }
...
def test_factory_creates_a1-voiceflow_ultravox_realtime_service():
    ...
    assert isinstance(service, A1 VoiceflowUltravoxRealtimeLLMService)
