...
from api.services.quota_service import check_a1-voiceflow_quota
...
async def start_campaign(
    ...
    # Check A1 Voiceflow quota before starting campaign (apply per-workflow
    # model_overrides so we evaluate the keys this campaign will use).
    quota_result = await check_a1-voiceflow_quota(user, workflow_id=campaign.workflow_id)
...
async def resume_campaign(
    ...
    # Check A1 Voiceflow quota before resuming campaign (apply per-workflow
    # model_overrides so we evaluate the keys this campaign will use).
    quota_result = await check_a1-voiceflow_quota(user, workflow_id=campaign.workflow_id)
    if not quota_result.has_quota:
        raise HTTPException(status_code=402, detail=quota_result.error_message)
