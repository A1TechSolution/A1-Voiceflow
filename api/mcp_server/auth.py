...
async def authenticate_mcp_request() -> UserModel:
    """Resolve the authenticated A1 Voiceflow user for an MCP tool invocation.
    ...
    and user identifiers. `_OrgRoutingExporter` reads `a1-voiceflow.org_id`
    ...
    if span.is_recording():
        ...
        # Intentionally NOT `a1-voiceflow.org_id` — that attribute triggers the
