...
async def _update_organization_usage(
    org, a1-voiceflow_tokens: float, duration_seconds: float, charge_usd: float | None
) -> None:
    """Update organization usage after a workflow run."""
    org_id = org.id
    await db_client.update_usage_after_run(
        org_id, a1-voiceflow_tokens, duration_seconds, charge_usd
    )
    if charge_usd is not None:
        logger.info(
            f"Updated organization usage with ${charge_usd:.2f} USD ({a1-voiceflow_tokens} A1 Voiceflow Tokens) and {duration_seconds}s duration for org {org_id}"
        )
    else:
        logger.info(
            f"Updated organization usage with {a1-voiceflow_tokens} A1 Voiceflow Tokens and {duration_seconds}s duration for org {org_id}"
        )
...
async def _build_usage_cost_snapshot(
    ...
    *,
    ...
    a1-voiceflow_tokens = float(total_cost_usd * Decimal("100"))
