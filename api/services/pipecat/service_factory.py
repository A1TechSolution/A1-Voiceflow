...
from pipecat.services.a1-voiceflow.llm import A1 VoiceflowLLMService
from pipecat.services.a1-voiceflow.stt import A1 VoiceflowSTTService, A1 VoiceflowSTTSettings
from pipecat.services.a1-voiceflow.tts import A1 VoiceflowTTSService, A1 VoiceflowTTSSettings
...
def create_stt_service(
    ...
    if user_config.stt.provider == ServiceProviders.DEEPGRAM.value:
        ...
        language = getattr(user_config.stt, "language", None) or "multi"
        return A1 VoiceflowSTTService(
            base_url=base_url,
            api_key=user_config.stt.api_key,
            settings=A1 VoiceflowSTTSettings(
                model=user_config.stt.model,
                language=language,
            ),
            keyterms=keyterms,
...
def create_tts_service(user_config, audio_config: "AudioConfig"):
    ...
    if user_config.tts.provider == ServiceProviders.DEEPGRAM.value:
        ...
        base_url = MPS_API_URL.replace("http://", "ws://").replace("https://", "wss://")
        return A1 VoiceflowTTSService(
            base_url=base_url,
            api_key=user_config.tts.api_key,
            settings=A1 VoiceflowTTSSettings(
                model=user_config.tts.model,
                voice=user_config.tts.voice,
                speed=user_config.tts.speed,
            ),
...
def create_realtime_llm_service(user_config, audio_config: "AudioConfig"):
    ...
    if provider == ServiceProviders.OPENAI_REALTIME.value:
        from api.services.pipecat.realtime.openai_realtime import (
            A1 VoiceflowOpenAIRealtimeLLMService,
        )
        ...
        return A1 VoiceflowOpenAIRealtimeLLMService(
            api_key=api_key,
            settings=A1 VoiceflowOpenAIRealtimeLLMService.Settings(
                model=model,
        ...
        from api.services.pipecat.realtime.grok_realtime import (
            A1 VoiceflowGrokRealtimeLLMService,
        )
        from pipecat.services.xai.realtime.events import SessionProperties

        return A1 VoiceflowGrokRealtimeLLMService(
            api_key=api_key,
            settings=A1 VoiceflowGrokRealtimeLLMService.Settings(
                model=model,
                session_properties=SessionProperties(
                    voice=voice or "Ara",
                ),
        ...
        from api.services.pipecat.realtime.ultravox_realtime import (
            A1 VoiceflowUltravoxOneShotInputParams,
            A1 VoiceflowUltravoxRealtimeLLMService,
        )

        return A1 VoiceflowUltravoxRealtimeLLMService(
            params=A1 VoiceflowUltravoxOneShotInputParams(
                api_key=api_key,
                model=model,
                voice=voice,
                output_medium="voice",
            ),
            settings=A1 VoiceflowUltravoxRealtimeLLMService.Settings(
                model=model,
                output_medium="voice",
            ),
        )
        ...
        return A1 VoiceflowGeminiLiveLLMService(
            api_key=api_key,
            settings=A1 VoiceflowGeminiLiveLLMService.Settings(**settings_kwargs),
        )
        ...
        if language:
            settings_kwargs["language"] = language
        return A1 VoiceflowGeminiLiveVertexLLMService(
            credentials=credentials,
            project_id=project_id,
            location=location,
            settings=A1 VoiceflowGeminiLiveVertexLLMService.Settings(**settings_kwargs),
        )
    else:
        raise HTTPException(
