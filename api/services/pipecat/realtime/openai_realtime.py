"""A1 Voiceflow subclass of pipecat's OpenAI Realtime LLM service.

Layers A1 Voiceflow engine integration quirks onto upstream-pristine
...
- **One-off LLMMessagesAppendFrame handling** for ephemeral realtime prompts
  like user-idle checks, without mutating A1 Voiceflow's local ``LLMContext``.
...
class A1 VoiceflowOpenAIRealtimeLLMService(OpenAIRealtimeLLMService):
    """OpenAI Realtime with A1 Voiceflow engine integration quirks. See module docstring."""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._user_is_muted: bool = False
        # A1 Voiceflow pre-populates self._context via the engine before the first
        ...
        # has finished speaking, matching A1 Voiceflow's Gemini Live behavior.
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TTSSpeakFrame):
            ...
            # lifecycle even when A1 Voiceflow has already pre-populated self._context.
