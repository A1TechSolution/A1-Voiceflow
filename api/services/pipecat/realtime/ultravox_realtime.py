"""A1 Voiceflow subclass of pipecat's Ultravox realtime LLM service.

Ultravox is audio-native and realtime, but prompt and tool configuration is
bound to call creation. A1 Voiceflow therefore cannot lean on in-session updates or
Gemini-style session resumption handles. This wrapper adapts Ultravox to the
A1 Voiceflow engine contract by:
...
- reconstructing Ultravox ``initialMessages`` from A1 Voiceflow context when the
  call must be recreated after a node transition
- appending a transient resumptive user nudge to recreated ``initialMessages``
  after tool-result transitions, without mutating A1 Voiceflow's stored context
- handling A1 Voiceflow-only frames such as user mute and idle append prompts
...
class A1 VoiceflowUltravoxOneShotInputParams(OneShotInputParams):
    """A1 Voiceflow-friendly OneShot params with string voice support."""
...
class A1 VoiceflowUltravoxRealtimeLLMService(UltravoxRealtimeLLMService):
    """Ultravox realtime with A1 Voiceflow engine integration quirks."""
    ...
    async def start(self, frame):
        # A1 Voiceflow defers call creation until the engine queues the node opening.
    ...
    def _build_one_shot_params(
        ...
        *,
        ...
        return A1 VoiceflowUltravoxOneShotInputParams(
