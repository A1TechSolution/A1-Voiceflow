...
async def create_transport(
    ...
    *,
    ...
    # not A1 Voiceflow's: ``inbound_encoding`` is what we *send into the call*
    # (A1 Voiceflow → Telnyx), and ``outbound_encoding`` is what we *receive out of
    # the call* (Telnyx → A1 Voiceflow).
    serializer = TelnyxFrameSerializer(
        stream_id=stream_id,
        call_control_id=call_control_id,
        api_key=api_key,
        inbound_encoding="PCMU",  # A1 Voiceflow → Telnyx; matches stream_bidirectional_codec
        outbound_encoding=encoding,  # Telnyx → A1 Voiceflow; from media_format.encoding
