...
# Configures git remotes (origin = your fork, upstream = Ali-Hamas/A1-Voiceflow),
...
UPSTREAM_URL="https://github.com/Ali-Hamas/A1-Voiceflow.git"
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║              A1 Voiceflow Contributor Bootstrap                    ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
canonical_https="https://github.com/Ali-Hamas/A1-Voiceflow.git"
canonical_ssh="git@github.com:Ali-Hamas/A1-Voiceflow.git"
...
if $needs_fork_prompt; then
    echo -e "${YELLOW}Enter your fork URL (e.g. https://github.com/<YOUR_HANDLE>/a1-voiceflow.git):${NC}"
    read -r -p "> " FORK_URL
    if [[ -z "$FORK_URL" ]]; then
...
if [[ -z "$existing_upstream" ]]; then
    ...
    echo -e "${YELLOW}Reset upstream to Ali-Hamas/A1-Voiceflow? [y/N]:${NC}"
