...
# Install a1-voiceflow API requirements first so pipecat's extras win on any
# shared transitive dependencies (matches api/Dockerfile and CI workflow).
echo "Installing a1-voiceflow API requirements..."
pip install -r api/requirements.txt

if [ "$DEV_MODE" -eq 1 ]; then
    echo "Installing a1-voiceflow API dev requirements..."
