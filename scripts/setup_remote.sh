...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║                   A1 Voiceflow Remote Setup                        ║"
...
if [[ -z "$SERVER_IP" ]]; then
    a1-voiceflow_fail "IP address cannot be empty"
fi

if ! a1-voiceflow_is_ipv4 "$SERVER_IP"; then
    a1-voiceflow_fail "Invalid IP address format"
fi
...
if [[ -z "${DEPLOY_MODE:-}" ]]; then
    if [[ -t 0 ]]; then
        ...
        echo "  1) prebuilt - pull official a1-voiceflow images (recommended, fastest)"
        ...
        case "$mode_choice" in
            ...
            *) a1-voiceflow_fail "invalid choice '$mode_choice'" ;;
        esac
    else
    ...
    if [[ "$REPO_SOURCE" == "clone" ]]; then
        if [[ -z "${FORK_REPO:-}" ]]; then
            if [[ -t 0 ]]; then
                ...
                read -p "[Ali-Hamas/A1-Voiceflow]: " FORK_REPO
                FORK_REPO="${FORK_REPO:-Ali-Hamas/A1-Voiceflow}"
            else
                FORK_REPO="Ali-Hamas/A1-Voiceflow"
            fi
        fi
...
if [[ "${DOGRAH_FORCE_OVERWRITE:-}" != "1" && "${DOGRAH_SKIP_DOWNLOAD:-}" != "1" ]]; then
    if [[ -f "$TARGET_DIR/.env" ]]; then
        ...
        echo ""
        echo -e "${YELLOW}Detected an existing A1 Voiceflow install:${NC}"
        ...
        echo -e "  ${BLUE}https://voiceflow-docs.a1techsolution.com/deployment/update${NC}"
        echo ""
...
if [[ "$DEPLOY_MODE" == "build" ]]; then
    if [[ "${DOGRAH_SKIP_DOWNLOAD:-}" == "1" ]]; then
        ...
        if [[ -e "a1-voiceflow" ]]; then
            a1-voiceflow_fail "'a1-voiceflow' directory already exists. Remove it or re-run with REPO_SOURCE=existing from inside it."
        fi
        echo -e "${BLUE}[1/$TOTAL] Cloning $FORK_REPO (branch: $BRANCH)...${NC}"
        git clone --branch "$BRANCH" --recurse-submodules "https://github.com/$FORK_REPO.git" a1-voiceflow
        cd a1-voiceflow
    ...
    if [[ "${DOGRAH_SKIP_DOWNLOAD:-}" != "1" ]]; then
        mkdir -p a1-voiceflow 2>/dev/null || true
        cd a1-voiceflow

        echo -e "${BLUE}[1/$TOTAL] Downloading deployment bundle...${NC}"
        curl -fsSL -o docker-compose.yaml "https://raw.githubusercontent.com/Ali-Hamas/A1-Voiceflow/main/docker-compose.yaml"
...
services:
  api:
    ...
    image: a1-voiceflow-local/a1-voiceflow-api:local
    pull_policy: never
  ...
  ui:
    ...
    image: a1-voiceflow-local/a1-voiceflow-ui:local
    pull_policy: never
OVERRIDE_EOF
...
echo "  - .env"
echo ""
echo -e "${YELLOW}To start A1 Voiceflow, run:${NC}"
echo ""
if [[ "$DEPLOY_MODE" != "build" || "${REPO_SOURCE:-}" != "existing" ]]; then
