...
# Requires:
#   - `python` in the `a1-voiceflow` conda env, `api/.env` sourced; the `api`
...
SPECS_JSON="$(mktemp -t a1-voiceflow-specs-XXXXXX.json)"
OPENAPI_JSON="$(mktemp -t a1-voiceflow-openapi-XXXXXX.json)"
...
PYTHONPATH="$REPO_ROOT/sdk/python/src" python -m a1-voiceflow_sdk.codegen \
    --input "$SPECS_JSON" \
    --out "sdk/python/src/a1-voiceflow_sdk/typed"
...
datamodel-codegen \
    ...
    --output "sdk/python/src/a1-voiceflow_sdk/_generated_models.py" \
...
python -m sdk.codegen.client_codegen \
    --input "$OPENAPI_JSON" \
    --py-out "sdk/python/src/a1-voiceflow_sdk/_generated_client.py" \
