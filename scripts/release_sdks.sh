#!/usr/bin/env bash
# Cut a release of both SDKs — a1-voiceflow-sdk (PyPI) and @a1-voiceflow/sdk (npm) —
...
#   - `npm login` completed as a member of the `a1-voiceflow` npm org. npm
...
if ! NPM_USER="$(npm whoami 2>/dev/null)"; then
    ...
    echo "       a1-voiceflow org before re-running this script — otherwise PyPI" >&2
...
if ! git diff --quiet -- sdk/python/src/a1-voiceflow_sdk/typed sdk/typescript/src/typed; then
    echo
...
echo
echo "============================================================"
echo "  Built a1-voiceflow-sdk==$VERSION and @a1-voiceflow/sdk@$VERSION"
echo "  Nothing has been published yet."
echo "============================================================"
echo

if confirm "Upload a1-voiceflow-sdk==$VERSION to TestPyPI first (recommended)?"; then
    (cd sdk/python && twine upload --repository testpypi dist/*)
    echo "  → https://test.pypi.org/project/a1-voiceflow-sdk/$VERSION/"
    echo
fi

if confirm "Publish @a1-voiceflow/sdk@$VERSION to npm? (will prompt for 2FA OTP)"; then
    (cd sdk/typescript && npm publish --access public)
    echo "  → https://www.npmjs.com/package/@a1-voiceflow/sdk/v/$VERSION"
    echo
fi

if confirm "Upload a1-voiceflow-sdk==$VERSION to PyPI?"; then
    (cd sdk/python && twine upload dist/*)
    echo "  → https://pypi.org/project/a1-voiceflow-sdk/$VERSION/"
    echo
fi

if confirm "Create annotated git tag sdks-v$VERSION at HEAD?"; then
    git tag -a "sdks-v$VERSION" -m "a1-voiceflow-sdk + @a1-voiceflow/sdk $VERSION"
