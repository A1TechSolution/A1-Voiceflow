...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║              A1 Voiceflow Custom Domain Setup                      ║"
echo "║     Automated Let's Encrypt SSL certificate setup            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

if [[ $EUID -ne 0 ]]; then
    a1-voiceflow_fail "This script must be run as root or with sudo"
fi

if [[ ! -d "a1-voiceflow" ]]; then
    echo -e "${RED}Error: 'a1-voiceflow' directory not found.${NC}"
    echo -e "${YELLOW}Please run this script from the directory containing your A1 Voiceflow installation.${NC}"
    echo -e "${YELLOW}If you haven't set up A1 Voiceflow yet, run the remote setup first:${NC}"
    echo -e "${BLUE}  curl -o setup_remote.sh https://raw.githubusercontent.com/Ali-Hamas/A1-Voiceflow/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh${NC}"
    exit 1
fi

echo -e "${YELLOW}Enter your domain name (e.g., voice.yourcompany.com):${NC}"
read -p "> " DOMAIN_NAME
[[ -n "$DOMAIN_NAME" ]] || a1-voiceflow_fail "Domain name cannot be empty"

if ! [[ "$DOMAIN_NAME" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
    a1-voiceflow_fail "Invalid domain name format"
fi

echo -e "${YELLOW}Enter your email address for SSL certificate notifications:${NC}"
read -p "> " EMAIL_ADDRESS
[[ -n "$EMAIL_ADDRESS" ]] || a1-voiceflow_fail "Email address cannot be empty (required by Let's Encrypt)"
...
if [[ -z "$SERVER_IP" ]]; then
    a1-voiceflow_warn "Warning: Could not detect server's public IP"
...
if command -v apt-get &> /dev/null; then
    ...
    a1-voiceflow_fail "Could not detect package manager. Please install certbot manually."
fi
echo -e "${GREEN}✓ Certbot installed${NC}"

echo -e "${BLUE}[3/7] Stopping A1 Voiceflow services...${NC}"
cd a1-voiceflow
DOGRAH_DEPLOY_PROJECT_DIR="$(pwd)"

if [[ ! -f remote_up.sh || ! -f scripts/lib/setup_common.sh ]]; then
    a1-voiceflow_download_remote_support_bundle "$(pwd)" "main"
fi

a1-voiceflow_require_init_compose_layout "$(pwd)"

if docker compose --profile remote ps --quiet 2>/dev/null | grep -q .; then
    docker compose --profile remote down
    echo -e "${GREEN}✓ A1 Voiceflow services stopped${NC}"
else
...
if [[ -z "${SERVER_IP:-}" ]]; then
    SERVER_IP="$(a1-voiceflow_infer_server_ip "$(pwd)" || true)"
fi

[[ -n "${SERVER_IP:-}" ]] || a1-voiceflow_fail "Could not determine SERVER_IP from the existing install"

a1-voiceflow_set_env_key .env SERVER_IP "$SERVER_IP"
a1-voiceflow_set_env_key .env PUBLIC_HOST "$DOMAIN_NAME"
a1-voiceflow_set_env_key .env PUBLIC_BASE_URL "https://$DOMAIN_NAME"
a1-voiceflow_delete_env_key .env BACKEND_URL
...
cat > /etc/letsencrypt/renewal-hooks/deploy/a1-voiceflow-reload.sh << HOOK_EOF
#!/bin/bash
...
HOOK_EOF
chmod +x /etc/letsencrypt/renewal-hooks/deploy/a1-voiceflow-reload.sh
...
echo ""
echo -e "${BLUE}[7/7] Starting A1 Voiceflow services through validated startup wrapper...${NC}"
./remote_up.sh
...
echo "  - a1-voiceflow/.env (canonical public host/base URL updated)"
echo "  - a1-voiceflow/certs/local.crt (SSL certificate)"
echo "  - a1-voiceflow/certs/local.key (SSL private key)"
echo "  - /etc/letsencrypt/renewal-hooks/deploy/a1-voiceflow-reload.sh (renewal hook)"
echo ""
