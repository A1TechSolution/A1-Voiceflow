...
REGISTRY="${REGISTRY:-ghcr.io/Ali-Hamas}"
...
if [[ "${DOGRAH_SKIP_DOWNLOAD:-}" != "1" ]]; then
    ...
    curl -sS -o docker-compose.yaml https://raw.githubusercontent.com/Ali-Hamas/A1-Voiceflow/main/docker-compose.yaml
    if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
        a1-voiceflow_download_init_support_bundle "$(pwd)" "main"
    fi
...
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    [[ -f scripts/run_a1-voiceflow_init.sh ]] || a1-voiceflow_fail "scripts/run_a1-voiceflow_init.sh not found. Re-run setup_local.sh without DOGRAH_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f scripts/lib/setup_common.sh ]] || a1-voiceflow_fail "scripts/lib/setup_common.sh not found. Re-run setup_local.sh without DOGRAH_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f deploy/templates/turnserver.remote.conf.template ]] || a1-voiceflow_fail "deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.sh without DOGRAH_SKIP_DOWNLOAD=1, or use a full repo checkout."
fi
...
cat > .env << ENV_EOF
# Container registry for A1 Voiceflow images
REGISTRY=$REGISTRY
...
echo ""
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    echo -e "${YELLOW}To start A1 Voiceflow with TURN, run:${NC}"
    echo ""
